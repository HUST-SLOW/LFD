...
from datasets import mvtec_ad, mvtec_3d_ad, visa_ad
...
class LFD():
    ...
    def _load_dataloader(self, dataset_name, class_name=None, train=True):
        ...
        elif dataset_name == 'visa_ad':
            if train:
                return visa_ad.visa_train_dataloader(self.cfg, class_name=class_name)
            else:
                return visa_ad.visa_test_dataloader(self.cfg, class_name=class_name)
        else:
            raise NotImplementedError("Dataset not implemented")

    def _train(self, dataset_name,class_name=None):
        ...
        self.train_dataloader = self._load_dataloader(dataset_name,
                                                      class_name=class_name,
                                                      train=True)
        if class_name is None:
            save_dir = self.cfg["outputs"]["ckpt_path"]
        else:
            save_dir = os.path.join(self.cfg["outputs"]["ckpt_path"], class_name)
        ...
        for epoch in self.global_progress:
            ...
            if (epoch + 1) % self.cfg['training']['save_epoch'] == 0 or epoch + 1 == self.cfg['training']['max_epochs']:
    ...
    def test(self, class_name=None):
        ...
        for class_name in classname_list:
            ...
            iou_px = np.mean(IoU_list)  # mean IoU
        ...
        # print the results of each class
